...
function objOfMatchesWithArray(array1, array2, callback) {
  ...
  array1.forEach((item1, index) => {
    let  changedValue = callback.reduce((value, cb) => cb(value), item1);
    if (changedValue === array2[index]) {
      finalObj[item1] = array2[index];
    }
  });
  return finalObj;
}
...
function objOfMatchesWithArray(array1, array2) {
  ...
  array1.forEach(value => {
    let values = array2.map(cb => cb(value));
    finalObj[value] = values;
  });
...
```js
function schedule(funcs,delays) {
  if (funcs.length !== delays.length) {
    console.log('invalid input');
    return;
  }

  for (let i = 0; i < funcs.length; i++) {
    setTimeout(funcs[i], delays[i] * 1000);
  }
}
